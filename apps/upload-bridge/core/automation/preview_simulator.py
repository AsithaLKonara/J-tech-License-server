...
class PreviewSimulator:
    ...
    def _apply_action(self, frame: Frame, code: str, parameters: Dict[str, object]) -> Frame:
        ...
        if code == "moveLeft1":
            ...
            result.pixels = self._rotate_180(result.pixels)
        elif code == "rotateCCW90":
            result.pixels = self._rotate_ccw_90(result.pixels)
        elif code == "mirrorH":
            ...
            # scrollText is a no-op in the preview simulator; it doesn't mutate the frame
            pass
    ...
    def _rotate_180(self, pixels: List[Tuple[int, int, int]]) -> List[Tuple[int, int, int]]:
        """Rotate 180 degrees."""
        # 180 is same as reverse
        return list(reversed(pixels))

    def _rotate_ccw_90(self, pixels: List[Tuple[int, int, int]]) -> List[Tuple[int, int, int]]:
    ...
    def _scroll_wrapping(self, frame: Frame, offset: int) -> Frame:
        """Apply wrapping horizontal scroll to a frame."""
        result = frame.copy()
        grid = self._frame_to_grid(result.pixels)
        new_grid = []
        
        for y in range(self.height):
            row = grid[y]
            # Wrap shift: (row[offset:] + row[:offset]) for left shift
            # But the 'speed' in LMS often moves text LEFT, so offset should be positive for left shift.
            new_row = row[offset % self.width:] + row[:offset % self.width]
            new_grid.append(new_row)
            
        result.pixels = self._grid_to_frame(new_grid)
        return result
