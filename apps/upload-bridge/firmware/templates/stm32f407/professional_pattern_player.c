/**
 * Professional STM32F407 LED Pattern Player - Upload Bridge Template
 * 
 * Full professional-grade pattern player with all advanced features
 * Optimized for STM32F407 with 1MB flash and 192KB RAM
...
#define MAX_LEDS 1000  // Optimized for STM32F407 memory (1MB flash, 192KB RAM)
...
// Speed keyframes (max 64 keyframes for STM32F407)
struct SpeedKeyframe {
    uint16_t frame;
    float multiplier;
};
SpeedKeyframe speed_keyframes[64];
...
int main(void) {
    ...
    for (uint8_t i = 0; i < num_keyframes && i < 64; i++) {
...
float apply_brightness_curve(float value, uint8_t curve_type) {
    ...
    switch (curve_type) {
        ...
        case BRIGHTNESS_CURVE_GAMMA_CORRECTED:
            // Full gamma correction for STM32F407
...
void system_clock_config(void) {
    ...
    // Configure PLL for 168MHz (STM32F407 max)
    RCC->PLLCFGR = (RCC->PLLCFGR & ~RCC_PLLCFGR_PLLM_Msk) | (8 << RCC_PLLCFGR_PLLM_Pos);
    RCC->PLLCFGR = (RCC->PLLCFGR & ~RCC_PLLCFGR_PLLN_Msk) | (336 << RCC_PLLCFGR_PLLN_Pos);
    RCC->PLLCFGR = (RCC->PLLCFGR & ~RCC_PLLCFGR_PLLP_Msk) | (0 << RCC_PLLCFGR_PLLP_Pos); // PLLP = 2
...
void ws2812_init(uint8_t pin) {
    ...
    // Configure pin as output, high speed
    GPIOA->MODER &= ~(0x3 << (pin * 2));
    GPIOA->MODER |= (0x1 << (pin * 2));
    GPIOA->OSPEEDR |= (0x3 << (pin * 2)); // High speed
}

void ws2812_send_byte(uint8_t byte) {
    // STM32F407 WS2812 bit-banging implementation (optimized for 168MHz)
    for (uint8_t i = 0; i < 8; i++) {
        if (byte & 0x80) {
            // Send 1 (0.9us high, 0.35us low)
            GPIOA->BSRR = (1 << DATA_PIN);
            for (volatile int j = 0; j < 30; j++); // ~0.9us at 168MHz
            GPIOA->BSRR = (1 << (DATA_PIN + 16)); // Reset bit
            for (volatile int j = 0; j < 12; j++);  // ~0.35us at 168MHz
        } else {
...
void ws2812_send(uint8_t *data, uint16_t len) {
    ...
    for (volatile int i = 0; i < 1680; i++); // ~50us at 168MHz
}

void delay_ms(uint32_t ms) {
    // Simple delay loop for STM32F407 at 168MHz
    for (uint32_t i = 0; i < ms; i++) {
        for (volatile int j = 0; j < 21000; j++); // ~1ms at 168MHz
    }
}
