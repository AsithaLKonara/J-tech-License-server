/**
 * Professional NUC123 LED Pattern Player - Upload Bridge Template
 * 
 * Full professional-grade pattern player with all advanced features
 * Optimized for NUC123 with 64KB flash and 20KB RAM
...
#define MAX_LEDS 200  // Optimized for NUC123 memory (64KB flash, 20KB RAM)
...
// Speed keyframes (max 12 keyframes for NUC123)
...
float apply_brightness_curve(float value, uint8_t curve_type) {
    ...
    switch (curve_type) {
        ...
        case BRIGHTNESS_CURVE_GAMMA_CORRECTED:
            // Full gamma correction for NUC123
...
void ws2812_send_byte(uint8_t byte) {
    // NUC123 WS2812 bit-banging implementation (optimized for 48MHz)
...
void delay_ms(uint32_t ms) {
    // Simple delay loop for NUC123 at 48MHz
