/**
 * Professional Nuvoton M051 LED Pattern Player - Upload Bridge Template
 * 
 * Full professional-grade pattern player with all advanced features
 * Optimized for Nuvoton M051 with 64KB flash and 4KB RAM
...
#define MAX_LEDS 200  // Optimized for Nuvoton M051 memory (64KB flash, 4KB RAM)
...
// Speed keyframes (max 12 keyframes for Nuvoton M051)
...
float apply_brightness_curve(float value, uint8_t curve_type) {
    ...
    switch (curve_type) {
        ...
        case BRIGHTNESS_CURVE_GAMMA_CORRECTED:
            // Full gamma correction for Nuvoton M051
...
void ws2812_send_byte(uint8_t byte) {
    // Nuvoton M051 WS2812 bit-banging implementation (optimized for 48MHz)
...
void delay_ms(uint32_t ms) {
    // Simple delay loop for Nuvoton M051 at 48MHz
