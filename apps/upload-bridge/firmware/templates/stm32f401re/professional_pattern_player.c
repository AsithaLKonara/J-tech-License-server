/**
 * Professional STM32F401RE LED Pattern Player - Upload Bridge Template
 * 
 * Full professional-grade pattern player with all advanced features
 * Optimized for STM32F401RE with 512KB flash and 96KB RAM
...
#define MAX_LEDS 500  // Optimized for STM32F401RE memory (512KB flash, 96KB RAM)
...
// Speed keyframes (max 32 keyframes for STM32F401RE)
...
int main(void) {
    ...
    for (uint8_t i = 0; i < num_keyframes && i < 32; i++) {
...
float apply_brightness_curve(float value, uint8_t curve_type) {
    ...
    switch (curve_type) {
        ...
        case BRIGHTNESS_CURVE_GAMMA_CORRECTED:
            // Full gamma correction for STM32F401RE
...
void system_clock_config(void) {
    ...
    // Configure PLL for 84MHz (STM32F401RE max)
...
void ws2812_send_byte(uint8_t byte) {
    // STM32F401RE WS2812 bit-banging implementation (optimized for 84MHz)
    for (uint8_t i = 0; i < 8; i++) {
        if (byte & 0x80) {
            // Send 1 (0.9us high, 0.35us low)
            GPIOA->BSRR = (1 << DATA_PIN);
            for (volatile int j = 0; j < 15; j++); // ~0.9us at 84MHz
            GPIOA->BSRR = (1 << (DATA_PIN + 16)); // Reset bit
            for (volatile int j = 0; j < 5; j++);  // ~0.35us at 84MHz
        } else {
...
void ws2812_send(uint8_t *data, uint16_t len) {
    ...
    for (volatile int i = 0; i < 700; i++); // ~50us at 84MHz
}

void delay_ms(uint32_t ms) {
    // Simple delay loop for STM32F401RE at 84MHz
    for (uint32_t i = 0; i < ms; i++) {
        for (volatile int j = 0; j < 10500; j++); // ~1ms at 84MHz
    }
}
